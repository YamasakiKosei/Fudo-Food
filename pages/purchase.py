...
try:
    # 戻るボタン
    if st.button(f'＜　{st.session_state.select_data["name"]}'): st.switch_page("pages/product_detail.py")

    # 購入商品
    col1, col2, col3, col4 = st.columns([1, 3, 1, 5])
    with col2:
        st.image(st.session_state.select_data["img"])                     # 画像
    with col4:
        st.subheader(f'**{st.session_state.select_data["name"]}**')       # 商品名
        col1, col2, col3 = st.columns([1.5,1,5])
        with col1: st.title(f'¥ {st.session_state.select_data["price"]}') # 価格
        with col2:
            st.write('')
            st.write('')
            st.write("送料込み")
    ...
    with col2:
        ...
        st.write("**還元予定ポイント**")
    with col3:
        st.subheader(f'**¥ {st.session_state.select_data["price"]-pt}**')         # 支払い金額
        st.subheader(f'**{int(st.session_state.select_data["price"]*0.01)}** pt') # 還元予定ポイント
    st.divider()
    ...
    with col2:
        with st.form(key="confirmation"):
            ...
            with col2: st.image(st.session_state.select_data["img"])                     # 画像
            with col4: st.write(f'**{st.session_state.select_data["name"]}**')           # 商品名
            col1, col2, col3, col4 = st.columns([2, 2, 1, 5])
            with col2:
                st.write("商品代金")
                st.write("ポイント")
            with col4:
                st.write(f'¥ {st.session_state.select_data["price"]}')                   # 価格
                st.write(f'ー {pt}')                                                     # ポイント使用
            st.divider()
            col1, col2, col3, col4 = st.columns([2, 2, 1, 5])
            with col2: st.write("**支払い金額**")
            with col4: st.subheader(f'**¥ {st.session_state.select_data["price"]-pt}**') # 支払い金額
